...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let followIntensity = Array.from(elements, el => el.getAttribute("fm-intensity"));
    ...
    function Update(){
        if (smoothPos.x !== mousePosCached.x || smoothPos.y !== mousePosCached.y) {
            ...
            for (let i = 0; i < followMouse.length; i++) {
                ...
                if (intensity) element.style.transform = `translate(${smoothPos.x * intensity}px, ${smoothPos.y * intensity}px)`;
            }
        }
